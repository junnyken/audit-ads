...
import type { AdAccount, AuditEntry, Paged } from '../../lib/types'
import { Card, EmptyState, ErrorState, Skeleton } from '../../components/ui'
import AuditDiff from '../../components/AuditDiff'
import { formatDateTime } from '../../lib/format'

export default function AuditTab({ account }: { account: AdAccount }) {
  const [page, setPage] = useState(1)
  const logs = useQuery({
    queryKey: ['account-audit', account.id, page],
    queryFn: () =>
      api.get<Paged<AuditEntry>>(
        `/api/v1/ad-accounts/${account.id}/audit-logs${query({ page, page_size: 25 })}`,
      ),
  })
  ...
  if (logs.data!.items.length === 0) {
    return <EmptyState title="No audit history" description="Mutations on this account will appear here." />
  }
